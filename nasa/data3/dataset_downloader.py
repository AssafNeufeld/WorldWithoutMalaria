    ...
    for month in range(1, 13):
        for day in range(1, 32):
            try:
                ...
                day = str(day).zfill(2)
                year = str(year)
                url = 'https://goldsmr4.gesdisc.eosdis.nasa.gov/data/MERRA2/M2SDNXSLV.5.12.4/' + year + '/' + month + '/MERRA2_300.statD_2d_slv_Nx.' + year + month + day + '.nc4'
                filename = 'MERRA2_300.statD_2d_slv_Nx.' + year + month + day + '.nc4'
                os.system('wget --user=barakyaari --password=malariaPass1 ' + url)
                readparamsFromFile(filename, {"T2MMAX", "T2MMIN", "T2MMEAN"})
                os.system('rm ' + filename)
                url = 'https://goldsmr4.gesdisc.eosdis.nasa.gov/data/MERRA2/M2T1NXSLV.5.12.4/' + year + '/' + month + '/MERRA2_300.tavg1_2d_slv_Nx.' + year + month + day + '.nc4'
                filename = 'MERRA2_300.tavg1_2d_slv_Nx.' + year + month + day + '.nc4'
                os.system('wget --user=barakyaari --password=malariaPass1 ' + url)
